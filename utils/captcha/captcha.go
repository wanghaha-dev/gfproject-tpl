...
func GetBase64Encode(codeLength int) (code string, result string) {
	code = getRandStr(codeLength)

	data := imgText(280, 100, code)
	result = base64.StdEncoding.EncodeToString(data)
	result = "data:image/jpg;base64," + result
	return
}

func SaveFile(filepath string, codeLength int) error {
	code := getRandStr(codeLength)
	data := imgText(280, 100, code)
